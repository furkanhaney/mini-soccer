...
class PygameRender:
    ...
    def draw_frame(self, state):
        player_positions, ball_position = self.decompose_state(state)
        for position in player_positions:
            x, y = (position * np.array(self.resolution)).astype(int)
            pygame.draw.circle(self.screen, (0, 255, 0), (x, y), 20)

        ball_x, ball_y = (ball_position * np.array(self.resolution)).astype(int)
        pygame.draw.circle(self.screen, (0, 0, 255), (ball_x, ball_y), 10)
